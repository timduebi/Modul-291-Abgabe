<template>
  <div class="box">
    <span v-if="!hasProduct()">Noch keine Proudkte</span>
    <div v-for="(product, index) in getProductsInCart"  :key="index" class="box-item">
      <img :src="product.image" alt="" class="item-thumb">
      <h3 class="item-name">{{ product.name }}</h3>
      <span class="item-amount">Anzahl: 1</span>
      <span class="item-price">CHF {{ product.price }}.00</span>
    </div>
    <div class="cart-info" v-if="hasProduct()">
      <span>Total: CHF {{ totalPrice() }}.00</span>
